...
const toBookingId = (booking) => booking._id;
...
const isConfirmationReady = (booking) => {
    const signatures = booking.signatures;
    return Boolean(signatures?.applicantPhoto &&
 ...
 */
const resolveBookerPhone = async (booking) => {
    try {
 ...
 */
export const buildBookingConfirmationVariables = (booking, contactNumber) => ({
...
const releaseClaim = async (bookingId) => {
    try {
        await Booking.updateOne({ _id: bookingId }, { $unset: { confirmationNotifiedAt: 1 } });
    }
    catch (error) {
...
export const notifyBookingConfirmed = async (booking) => {
    try {
        ...
        const claimed = await Booking.findOneAndUpdate({ _id: bookingId, confirmationNotifiedAt: { $exists: false } }, { $set: { confirmationNotifiedAt: new Date() } }, { new: true });
        if (!claimed) {
            return { sent: false, skipped: "already-notified" };
        }
        const bookerPhone = await resolveBookerPhone(claimed);
        const variables = buildBookingConfirmationVariables(claimed, bookerPhone || recipient);
