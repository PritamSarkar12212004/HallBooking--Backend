...
export const sendSms = async (mobileNumber, smsData = {}) => {
    try {
        const { SMS_USER, SMS_PASSWORD, SMS_CHANNEL, SMS_ROUTE, SMS_DLT_TEMPLATE_ID, SMS_PEID, SMS_SENDER_ID, SMS_OTP_TEMPLATE_ID, } = process.env;
        // -----------------------------------------
        // Validate SMS credentials
        // -----------------------------------------
        if (!SMS_USER || !SMS_PASSWORD) {
            throw new ApiError(500, "SMS_USER and SMS_PASSWORD are missing in .env");
        }
        // -----------------------------------------
        ...
        const senderId = smsData.senderId ||
            ...
            "";
        if (!senderId) {
            throw new ApiError(500, "SMS_SENDER_ID is missing in .env");
        }
        // -----------------------------------------
        // DLT Template ID
        // -----------------------------------------
        const templateId = smsData.templateId ||
            ...
            "";
        if (!templateId) {
            throw new ApiError(500, "SMS DLT Template ID is missing in .env");
        }
        // -----------------------------------------
        // SMS Message
        // -----------------------------------------
        ...
        const url = `http://neo.c2sms.com/api/mt/SendSMS?${params}`;
        // -----------------------------------------
        // Logging
        // -----------------------------------------
        ...
        return await new Promise((resolve, reject) => {
            const req = http.get(url, {
                ...
                res.on("end", () => {
                    ...
                    try {
                        ...
                        const parsedErrorCode = parsed.ErrorCode ?? parsed.error;
                        gatewayError =
            ...
            // -----------------------------------------
            req.on("error", (error) => {
                logger.error("C2SMS request error", {
                ...
                if (error.code ===
                    ...
                    reject({
                        success: false,
                        message: "Connection reset by SMS server. Please check network/firewall settings or contact the SMS provider.",
                        code: "ECONNRESET",
                    });
                    return;
                }
            ...
            // -----------------------------------------
            req.on("timeout", () => {
                logger.error("C2SMS request timeout");
                req.destroy();
                reject({
                    success: false,
...
export const sendOtpSms = async ({ phone, code, }) => {
    ...
    // -----------------------------------------
    const otpExpirySeconds = Number(process.env.OTP_EXPIRATION_SECONDS) || 300;
    const defaultMessage = `Your OTP is {code}. Please use this OTP to log in. ` +
        `Do not share this OTP with anyone. Regards, Unisol Services LLP`;
    const templateMessage = process.env.SMS_OTP_MESSAGE?.trim() || defaultMessage;
    ...
    try {
        ...
        // -----------------------------------------
        if (!result.success) {
            throw new ApiError(502, `SMS gateway error (${result.statusCode ?? "unknown"}): ${result.response ||
                result.message ||
                "No response"}`);
        }
