...
import type {
    ...
    ListApplicantsQuery,
    PaginatedApplicants,
} from "./applicant.type.js";

/** Statuses that still count as a live/active engagement with the applicant. */
const ACTIVE_STATUSES: BookingStatus[] = ["Pending", "Office-Approved", "Confirmed"];

const trimString = (path: string) => ({
...
const sortStage = (sort: ApplicantSort): PipelineStage => {
    switch (sort) {
        case "oldest":
...
const buildPipeline = (params: ListApplicantsQuery): PipelineStage[] => {
...
interface ApplicantFacet {
    data?: RawApplicantRow[];
    meta?: { total?: number }[];
}
...
export const listApplicants = async (
    ...
    const result = await Booking.aggregate<ApplicantFacet>(pipeline as unknown as PipelineStage[])
        .allowDiskUse(true);
    ...
    const applicants: ApplicantListItem[] = rows.map((row) => ({
        id: row.id,
        name: row.name || "N/A",
        mobile: row.mobile || "",
        image: row.image || "",
        ...
        latestStatus: (row.latestStatus || "") as BookingStatus | "",
    }));
